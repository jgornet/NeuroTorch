...
class TestTiffDataset(unittest.TestCase):
    def test_tiff_dataset(self):
        ...
        self.assertTrue((tif.imread("./tests/datasets/test_sample.tif")
                         == testDataset[79]).all,
                        "TIFF dataset value does not match correct value")

        # Test that TiffDataset can read and write consistent samples
        tif.imsave("./tests/datasets/test_write.tif", testDataset[79])
        self.assertTrue((tif.imread("./tests/datasets/test_write.tif")
                         == testDataset[79]).all,
